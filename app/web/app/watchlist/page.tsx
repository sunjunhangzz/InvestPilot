...
type Wl = {
  ...
  entryPrice: number | null; latestPrice: number | null;
  trackingDays: number; firstRecommendedDate: string; exitReason: string | null;
  returnPct: number | null; maxDrawdownPct: number | null; trendBreach: boolean | null;
};
  ...
  return (
    <AppShell><section className="mx-auto max-w-7xl px-6 py-8"><h2 className="text-xl font-semibold">观察池</h2>
      {items.length === 0 ? <div className="mt-6 rounded-lg border border-[var(--line)] bg-white p-6 text-sm text-[var(--muted)]">暂无观察股票</div> :
        <div className="mt-4 overflow-x-auto rounded-lg border border-[var(--line)] bg-white"><table className="w-full text-sm"><thead><tr className="border-b border-[var(--line)] bg-[var(--panel)] text-left text-[var(--muted)]"><th className="px-3 py-3 font-medium" title="active=观察中 downgraded=降级 exit=退出 blocked=阻断">状态 ⓘ</th><th className="px-3 py-3 font-medium">代码</th><th className="px-3 py-3 font-medium">名称</th><th className="px-3 py-3 font-medium" title="首次推荐时的收盘价">入场价 ⓘ</th><th className="px-3 py-3 font-medium" title="最近交易日收盘价">最新价 ⓘ</th><th className="px-3 py-3 font-medium" title="推荐后的累计涨跌幅。绿色=盈利，红色=亏损">收益 ⓘ</th><th className="px-3 py-3 font-medium" title="入场后最大亏损幅度。<-10%需关注">最大回撤 ⓘ</th><th className="px-3 py-3 font-medium" title="最新价是否跌破MA20或MA60。是=趋势转弱">破趋势线 ⓘ</th><th className="px-3 py-3 font-medium" title="从首次推荐到现在的交易日数（5-20天）">跟踪天数 ⓘ</th><th className="px-3 py-3 font-medium" title="退出观察池的原因">退出原因 ⓘ</th></tr></thead><tbody>
          {items.map((w) => (<tr key={w.code} className="border-b border-[var(--line)] hover:bg-[var(--panel)]"><td className="px-3 py-3">{sl(w.status)}</td><td className="px-3 py-3"><Link className="font-medium text-[var(--accent)] hover:underline" href={`/stocks/${w.code}`}>{w.code}</Link></td><td className="px-3 py-3">{w.name}</td><td className="px-3 py-3">{w.entryPrice?.toFixed(2) ?? "—"}</td><td className="px-3 py-3">{w.latestPrice?.toFixed(2) ?? "—"}</td><td className={`px-3 py-3 ${(w.returnPct ?? 0) > 0 ? "text-red-600" : "text-green-600"}`}>{w.returnPct != null ? `${w.returnPct > 0 ? "+" : ""}${w.returnPct}%` : "—"}</td><td className="px-3 py-3 text-red-500">{w.maxDrawdownPct != null ? `${w.maxDrawdownPct}%` : "—"}</td><td className="px-3 py-3">{w.trendBreach === true ? "⚠️ 是" : w.trendBreach === false ? "否" : "—"}</td><td className="px-3 py-3">{w.trackingDays}</td><td className="px-3 py-3 text-[var(--muted)]">{w.exitReason || "—"}</td></tr>))}
        </tbody></table></div>}
    </section></AppShell>
  );
}

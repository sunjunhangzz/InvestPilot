...
```
第1轮（独立分析）：
  5 个 Agent 同时独立分析 → 各自输出 rating + argument
  → 如果 5 票全票一致 → 直接给主席（罕见）
  → 否则进入第 2 轮

第2轮（交叉质疑）：
  每个 Agent 看到其他 4 人的第 1 轮意见 → 重新评估
  → 可以坚持或修改自己的 rating
  → 必须对与自己意见不一致的 Agent 做出回应

主席裁决：
  综合两轮所有发言 → 投票统计 → 输出最终 rating + consensus
```

**早停规则**：仅当第 1 轮 5 票全票一致时跳过第 2 轮。5 个不同视角几乎不可能全票一致，保证至少 2 轮辩论。
...
每只股票 10-11 次调用（2 轮 × 5 Agent + 主席），50 只 × 400 token × DeepSeek 价格 ≈ **0.02 元/天**。

5 个不同视角几乎不可能全票一致，保证至少 2 轮辩论。实际耗时：50 只 × 11 次 × ~0.5 秒 = ~4.5 分钟。

...
**搜索配额**：Tavily 免费 1000 次/月。10 只 × 5 次 = 50 次/天 ≈ **20 天/月**，绰绰有余。SerpAPI 作备援。

## 对推荐的影响

### AI 投票定级规则

规则推荐按评分选出 Top 10 只。AI 委员会对全部 10 只进行四轮辩论投票：
...
每只股票 ~23 次调用（4 轮 × 5 Agent + 分组代表 + 主席），**仅对规则评分前 10 只执行 Agent 评估**。

10 只 ≈ 230 次 LLM 调用/天。**日均费用 ~0.09 元，月费 ~3 元**。耗时：10 只 × ~12 秒/只 ≈ **2 分钟**。

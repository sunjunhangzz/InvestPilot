...
第3轮（网络举证——仅前10+争议股）：
  仅对规则评分前 10 名 + Agent 评级分歧 ≥2 分的股票执行搜索
...
**搜索配额**：Tavily 免费 1000 次/月。仅对前 10 名 + 争议股票搜索（~12 只 × 5 次 = 60 次/天），≈ 16 天用量。SerpAPI 100 次/月作备援。

## 对推荐的影响

### AI 投票定级规则

规则推荐按评分选出 Top 50 只（A=前10，B=11-50）。AI 委员会对全部推荐投票后：

| 规则评级 | AI 投票 | 最终定级 | 说明 |
|---|---|---|---|
| 任意 | 得票 Top 5（5票中至少3票同意） | **AI-A** | 最多 5 只，AI 认证的最优 |
| A | 得票 6-10 名 | A | 规则评级保持 |
| A | ≤2票同意 | B | AI 否决，降级 |
| B | ≥3票同意 | A | AI 发掘，升级 |

**AI-A 数量上限 = 5 只**。超过时按得票数取前 5。

在推荐列表中新增「AI 评级」列。AI-A（最多5只）优先展示，被降级/升级的有标记。
...
每只股票 ~23 次调用（4 轮 × 5 Agent + 分组代表 + 主席），50 只 ≈ 1150 次 LLM 调用/天。

**日均费用 ~0.46 元，月费 ~14 元**。实际耗时：50 只 × ~12 秒/只 ≈ **10 分钟**。

第 3 轮搜索**仅对前 10 名 + 争议股票（Agent 评级分歧 ≥2 分）执行**，其余跳过。Tavily 配额从 4 天延长至 ~20 天。
...
每一轮辩论结束后生成 Markdown 报告，存入 `reports/debates/` 目录。

**报告格式示例**：

```markdown
# Agent 委员会辩论报告 — 000001 平安银行
> 交易日 2026-07-03 | 规则评级 A | 趋势 85 基本面 72

## 第1轮：独立分析

| Agent | 评级 | 观点 |
|---|---|---|
| 技术面 | 4 | MA多头排列，短期超买需关注 |
| 基本面 | 3 | 营收增速-10.4%，增长乏力 |
| 风险官 | 2 | 波动率偏高，回撤-10.6% |
| 估值师 | 3 | PE/PB 缺失，估值数据不足 |
| 行业对比 | 4 | 行业中位数之上 |

## 第2轮：交叉质疑

**技术面 → 基本面**：营收增速确实是隐忧，但技术面强势可以弥补…

**基本面 → 技术面**：技术面支撑有限，营收持续下滑不能忽视…

**风险官 → 技术面**：高波动+10%回撤，风险回报比不理想…

## 第3轮：网络举证

- 技术面引用：[平安银行 2026 一季报：营收利润同比双增](https://bank.pingan.com/...)
- 基本面引用：[平安银行 ROE 连续三年低于行业均值](https://...)
- 风险官引用：[银行业整体不良率上升 2026](https://...)

## 第4轮：分组辩论

**看多组（技术面+行业对比）**：趋势强劲+行业领先，短期波动可接受

**看空组（基本面+风险官+估值师）**：增速乏力+高波动+估值不明，风险大于机会
...
> 主席总结：技术面支撑但基本面和风险都存在明显问题。3票看空 vs 2票看多。
```

## 存储

- 路径：`reports/debates/{trade_date}/{code}.md`
- 同时在 `agent_reports` 表中存储摘要（rating/consensus/votes）
- Web 端通过 stock API 返回报告路径，前端展示链接
